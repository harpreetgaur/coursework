...
`r  year_counts %>% summarize(n())`
`r  total_counts %>% summarize(n())`
...
combined <- left_join(total_counts,year_counts, by='year')

combined <- combined %>% mutate(proportion = volume/total_volume)
...
over_time <- combined %>% 
  ...
  mutate(term_factored = 
  factor(term, levels = c('1883','1910','1950')))%>%
  ggplot(aes(x=year, y=proportion, color=term_factored))+
  geom_line() + coord_cartesian(xlim=c(1850,2012)) +
  scale_y_continuous(lab = percent)
...
raw_mentions <- combined %>% 
  filter(term=='1883'|term=='1910'|term=='1950') %>% 
  mutate(term_factored = 
  factor(term, levels = c('1950','1910','1883')))%>%
  ggplot(aes(x=year, y=volume, color=term_factored))+
  geom_line() + coord_cartesian(xlim=c(1850,2012)) 
...
```{r plot-totals}
#combined <- combined %>%filter(term=='1883'|term=='1910'|term=='1950')
combined

total_plot<-combined %>% group_by(year) %>%
  summarize(count=sum(total_volume)) %>%
...
interm_df<- combined %>% 
  ...
  mutate(peakyear=year) %>% mutate(hl=(proportion/2))
interm_df
...
df1 <- combined %>% select(term, proportion, year)
df2 <- interm_df %>% select(term, peakyear)



half_life <- left_join(df1,df2, by="term") %>% 
  filter(term>=1875 & term<=1975) %>% 
  filter(year>=peakyear) %>% 
  group_by(term) %>%
  filter(proportion<=hl)%>%
  mutate(year_min=min(year)-peakyear) %>% ungroup()
...
half_life %>% 
  ggplot(aes(x = term, y = year_min)) + geom_point() + geom_smooth(method='loess')
